        ...
        <div className='px-4 py-6 sm:px-6 lg:px-8 lg:py-8'>
            <section>
                <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                    ...
                    <div>
                        <button className='flex items-center gap-2 text-sm text-accent sm:text-base'>
                            View all →
                        </button>
                    </div>
                </div>

                <div className='mt-6 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6'>
                    {products.map((product) => (
                        <Link to={`/products/${product.id}`} key={product.id} className='group flex flex-col overflow-hidden rounded-lg border border-transparent'>
